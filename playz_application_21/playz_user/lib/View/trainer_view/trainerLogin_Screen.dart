...
import 'package:playz_user/View/trainer_view/trainerForgotPassword_Screen.dart';
import 'package:playz_user/View/trainer_view/trainerRegister_Screen.dart';
import 'package:playz_user/View/trainer_view/trainer_navigation.dart';
import 'package:playz_user/View/worker_view/workerForgotPassword_Screen.dart'; // ADDED FIREBASE AUTH
...
setState(() {
  isLoading = true;
    ...
    try {
      ...
      String modifiedPassword = "${passwordInput}Turf_Trainer";
      ...
      TrainerSettings.saveEmail(userCredential.user?.email ?? '');
      TrainerSettings.saveUserId(userCredential.user?.uid ?? '');
      TrainerSettings.saveIsLoggedIn(true);

      final userDoc = await _firestore.collection('Turf_Trainer')
      .doc(email)
      .get();

      TrainerSettings.saveTrainerName(userDoc['userName'] ?? "Trainer");

      // 3. Success Handling
      _showSnackbar("Login Successfully", Colors.deepOrange);
  ...
  void _goToForgotPassword() {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => const TrainerForgotPasswordScreen(),
      ),
    );
  }
