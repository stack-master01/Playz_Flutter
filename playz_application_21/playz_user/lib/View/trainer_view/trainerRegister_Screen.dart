...
import 'package:playz_user/View/trainer_view/screen/welcome_screen.dart';
...
class UserWelcomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const Scaffold(
      body: Center(child: Text("Welcome! Proceed to User Home.")),
    );
  }
}
...
class _TrainerRegisterScreenState extends State<TrainerRegisterScreen> {
  ...
  final List<String> _networkBackgroundUrls = [
    "https://images.unsplash.com/flagged/photo-1597786772028-7859f6dcb903?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1yZWxhdGVkfDIyfHx8ZW58MHx8fHx8&auto=format&fit=crop&q=60&w=600",
   "https://images.unsplash.com/photo-1606328500899-38351f33df8a?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&q=80&w=687",
   "https://images.unsplash.com/photo-1584466977773-e625c37cdd50?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&q=80&w=687",
  ];
...
setState(() {
  isLoading = true;
    ...
    try {
      ...
      String password = "${_passwordController.text}Turf_Trainer";
      ...
      TrainerSettings.saveEmail(_emailController.text.trim());
      TrainerSettings.saveIsLoggedIn(true);
      TrainerSettings.saveUserId(userCredential.user!.uid);
      TrainerSettings.saveTrainerName(_usernameController.text.trim());
TrainerSettings trainer = await TrainerSettings().loadSettings();
      log("shared Email: ${trainer.trainerEmail}");
 ...
 final DateTime now = DateTime.now();
       await _firestore.collection("Turf_Trainer").doc(_emailController.text.trim()).set({
      ...
      await _firestore.collection('Turf_Trainer').doc(_emailController.text.trim()).collection('Trainer_Data').doc('Profile_Data').set({
        "about":"",
        "contact_no":"",
        "email":"",
        "location":{},
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (context) => const WelcomeScreen(), // Change to your intended home screen
        ),
      );
  ...
  void _goToLogin() {
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => TrainerLoginScreen(),
      ),
    );
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
                          ...
                          GestureDetector(
                            ...
                            child: Text(
                              ...
                              style: TextStyle(
                                fontSize: 15,
                                fontWeight: FontWeight.bold,
                                color: Colors.deepOrange, // Highlighting the action link
                              ),
                            ),
                          ),
                        ],
          ...
          ),if(isLoading) WorkerLoaderScreen(),
        ],
      ),
    );
  }
