...
class TrainerChat extends StatefulWidget {
  ...
  TrainerChat({super.key, required this.groupDataMap});
...
class _TrainerChatState extends State<TrainerChat> {
  ...
  void initState() {
    ...
    groupNameKey =
        currentGroupDataMap['trainer_data']['trainer_name'] ?? "name";
    groupImage =
        currentGroupDataMap['trainer_data']['trainer_images'][0] ?? "image url";
                            ...
                            child: StreamBuilder<List<Map<String, dynamic>>>(
                              ...
                              stream: UserGroupChatController()
                                  .streamCentralGroupChats(
                                    groupId:
                                        currentGroupDataMap['trainer_data']['groupID'],
                                  ),
                              builder: (context, snapshot) {
                                ...
                                final allTrainerChats = snapshot.data!;

                                // 1. Process Timestamps and create chatCards (Your old loadAllChats logic)
                                List<Map<String, dynamic>> processedChats = [];

                                // This 'for' loop is CORRECT because allTrainerChats is an Iterable (List)
                                for (var newItem in allTrainerChats) {
                                ...
                                return Padding(
                                  ...
                                  child: ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      // --- Message Map Handling (your text logic + new list logic) ---
                                      ...
                                      try {
                                        // Try to parse message as JSON array (list of exercises)
                                        final decoded = jsonDecode(messageKey);
                                        if (decoded is List &&
                                            decoded.isNotEmpty &&
                                            decoded.first is Map) {
                                          // ✅ It's a list of exercises → show cards
                                          ...
                                          // ❌ Not a valid list — fallback to normal text
                                          throw FormatException();
                                        }
                                      } catch (_) {
                                        // 🔤 Normal text logic (your original bubble)
                                        messageWidget = Column(
                                          crossAxisAlignment:
                                              CrossAxisAlignment.start,
                                          children: [
